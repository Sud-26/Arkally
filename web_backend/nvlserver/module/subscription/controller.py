...
from .service import (
    ...
    get_rebate_element, update_rebate_element, delete_rebate_element, get_rebate_dropdown_list, update_subscription_module
)
...
async def api_subscription_get(request: Request, user):
    ...
    rebate_id = proc_arg_to_int(request.args.get('rebate_id', 0),0)
            ...
            if user:
                if user.get('user_id', None):
                    if user.get('account_type_name') in ['admin', 'billing']:
                        user_id = proc_arg_to_int(user_id,0)
                        #user_id =user.get('user_id', None)                        
                    else:
                        rebate_id =proc_arg_to_int(user.get('user_id', 0),0)
                        user_id = 0
                    subscription_list = await get_subscription_list(
                        request,description=description, subscription_model_id=subscription_model_id,
                        user_id=user_id, rebate_id=rebate_id, limit=size, offset=offset)
                    subscription_list_count = await get_subscription_list_count(
                        request, description=description, subscription_model_id=subscription_model_id, user_id=user_id, rebate_id=rebate_id)
            ...
            if user:
                if user.get('user_id'):
                    if user.get('account_type_name') in ['admin', 'billing']:
                        ...
                        hw_module_id = request.json.get('hw_module_id', None)
                        ...
                        if True and date_from_front and date_to_front:
                            ...
                            subscription_element = await create_subscription_element(
                                ...
                                subscription_model_id=subscription_model_id, hw_module_id=hw_module_id,
...
# RUN EVERY DAYE FOR CHECKING SUBSCRIPTION OF MOUDLE DEACTIVE
@api_subscription_blueprint.route('/subscription_deactive_bydate', methods=['GET'])
async def api_subscription_updatesubcription(request: Request):
    """
    :param request:
    :param user:
    :return:
    ...
    if request.method == 'GET':
        try:
            ...
            await update_subscription_module(request, current_date)                        
            status = 200
            ret_val['success'] = True
            ret_val['message'] = 'server.accepted'
        except Exception as al_err:
            logger.error('Function api_subscription_updatesubcription -> GET erred with: {}'.format(al_err))
    return response.raw(
        ujson.dumps(ret_val).encode(),
        headers={'X-Served-By': 'sanic', 'Content-Type': 'application/json'},
        status=status
