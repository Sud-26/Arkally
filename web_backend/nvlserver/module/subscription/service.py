...
from .specification.get_subscription_specification import (
    get_subscription_list_query, get_subscription_list_count_query, get_subscription_element_query, get_expir_subscription_element_query
)
from .specification.create_subscription_specification import create_subscription_element_query
from .specification.update_subscription_specification import update_subscription_element_query, update_expir_subscription_element_query
...
async def get_subscription_list(
    ...
    try:
        if limit > 0:
            query_str += ' ORDER BY sub.id DESC LIMIT $5 OFFSET $6;'
            async with request.app.pg.acquire() as connection:
                ...
                rows = await connection.fetch(query_str, description, subscription_model_id, user_id, rebate_id, limit, offset)
        else:
            ...
            async with request.app.pg.acquire() as connection:
                ...
                rows = await connection.fetch(query_str, description, subscription_model_id, user_id, rebate_id)
...
async def get_subscription_list_count(
    ...
    try:
        ...
        async with request.app.pg.acquire() as connection:
            row = await connection.fetchval(query_str, description,  subscription_model_id, user_id, rebate_id)
            if row is not None:
                ret_val = row
    ...
    try:
        ...
        async with request.app.pg.acquire() as connection:
            row = await connection.fetchrow(
                query_str, subscription_uuid, user_id, subscription_model_id, hw_module_id,
                unit_count, date_from, date_to, active)
            if row is not None:
                dta = dict(row)
...
async def update_subscription_element(
    ...
    try:
        ...
        async with request.app.pg.acquire() as connection:
            row = await connection.fetchrow(
                query_str, subscription_id, user_id, subscription_model_id, hw_module_id,
                unit_count, date_from, date_to, active)
            if row is not None:
                dta = dict(row)
                ret_val = await get_subscription_element(request, dta.get('id'))
...
async def update_subscription_module(
        request: Request,
        currentdate: str ="") -> dict:
    """ Delete rebate element
    ...
    ret_val = {}
    query_str = get_expir_subscription_element_query
    query_str2 = update_expir_subscription_element_query
    try:

        async with request.app.pg.acquire() as connection:
            rows = await connection.fetch(query_str, currentdate)
            if rows is not None:
                ret_val = [dict(x) for x in rows]
                for x in ret_val:
                    hw_module_id=x['hw_module_id']
                    active=False
                    row = await connection.fetchrow(
                    update_expir_subscription_element_query, hw_module_id, active, currentdate)
    except Exception as cleerr:
        logger.error('update_subscription_module service erred with: {}'.format(cleerr))
