...
from .specification.create_user_specification import create_user_element_query ,create_user_element_query_front
...
# CREATE CUSTOMER Front end
async def create_user_element_front(
        request: Request,
        email: str = '',
        password: str = '',
        fullname: str = '',
    ...
    ret_val = {}
    query_str = create_user_element_query_front
    try:
        async with request.app.pg.acquire() as connection:
            timezone_element = await get_timezone_element(request, timezone_id=timezone)
            ...
            row = await connection.fetchrow(
                ...
                account_type_id, active, gendar, companyName, city, address, postalCode, country, mobileNumber, webPage, updateByMails ,vatId)
            if row is not None:
                temp_dta = dict(row)
                await update_user_permission_list(
                    request, temp_dta.get('id'), temp_dta.get('account_type_id'))
        ...
        logger.error('create_user_element_query_front service erred with: {}'.format(ccerr))
