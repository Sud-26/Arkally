...
from web_backend.backend_services.email_send_service import compose_email , send_email
...
from .service import (
    ...
    update_user_element_map_pool_time, create_user_element_front,update_user_element_verification_status,
    update_user_element_verification_code, check_user_element_verification_code, update_user_profile
)
api_user_management_blueprint = Blueprint('api_user_management', url_prefix='/api/user_management')

# Sanic_Mail.SetConfig(
#     app,
#     MAIL_SENDER="augursmail@gmail.com",
#     MAIL_SENDER_PASSWORD="Augurs@321",
...
@api_user_management_blueprint.route('/create_user_front', methods=['POST'])
#@inject_user()
#@scoped(['user:create'], require_all=True, require_all_actions=True)
async def api_user_post_create(request: Request):
    """ Create User
    ...
    """
    status = 500
    ret_val = {'success': False, 'message': 'Oops Some thing wrong', 'data': None}
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            language_id = request.json.get('language_id', 1)
            locked = request.json.get('locked', False)
            account_type_id = request.json.get('account_type_id', 3)
            timezone = request.json.get('timezone_id', None)
            map_pool_time = request.json.get('map_pool_time', None)
            gendar = request.json.get('gendar', None)
            companyName = request.json.get('companyName', None)
            address = request.json.get('address', None)
            city = request.json.get('city', None)
            postalCode = request.json.get('postalCode', None)
            country = request.json.get('country', None)
            mobileNumber = request.json.get('mobileNumber', None)
            webPage = request.json.get('webPage', None)
            updateByMails = request.json.get('updateByMails', None)
            vatId = request.json.get('vatId', None)
            active = request.json.get('active', False)
            ...
            if None not in [email, q_pass, fullname, timezone, language_id, account_type_id]:
                ...
                if email_unique:
                    if q_pass:
                        user_element = await create_user_element_front(
                            request, email=email, password=str(q_pass), fullname=fullname,
                            locked=locked, language_id=language_id,
                            timezone=timezone, map_pool_time=map_pool_time, account_type_id=account_type_id,
                            active=active, gendar=gendar, companyName=companyName, address=address, city=city, postalCode=postalCode,
                            country=country, mobileNumber=mobileNumber, webPage=webPage, updateByMails=updateByMails, vatId=vatId)
                        if user_element:
                            user_data = await get_user_by_id(request, user_element.get('id'))
                            await update_user_in_redis(request, user_element.get('id'))
                            # attachments = {}
                            ...
                            #     content="<html><body>Please verify your email on click below link<br/><a href='#'>Verify email</a></body></html>",
                    ...
                    status = 412
                    ret_val['message'] = 'Email already exists'
            else:
                status = 412
                ret_val['message'] = 'Please insert require fields'
                ...
                if user.get('user_id', None):
                    ...
                    if None not in [email, fullname, timezone, language_id] and email_unique:
                        user_element = await update_user_element(
                            ...
                            account_type_id=account_type_id, active=active, address=address,
                            city=city, companyname=companyname, country=country, gendar=gendar,
                            postalcode=postalcode, webpage=webpage, vatid=vatid
                            )
...
@api_user_management_blueprint.route('/email_verification', methods=['POST'])
async def api_user_verify_email(request: Request):
    """ verify user email
    :param request:
    :param user_id:
    :return:
    ...
    if request.method == 'POST':
        ...
        try:
            user_data = await get_user_by_id(request, request.json.get('user_id',None))
            if user_data:
                user = await update_user_element_verification_status(request, request.json.get('user_id',None))
                if user:
                    ret_val['success'] = True
                    status = 202
                    ret_val['message'] = 'server.accepted'
...
@api_user_management_blueprint.route('/forgot_password_email', methods=['POST'])
async def api_user_forgot_password_email(request: Request):
    """ send otp user email
    :param request:
    :param email:
    :return:
    ...
    if request.method == 'POST':
        ...
        """ send otp in email"""
        try:
            email_unique = await check_unique_by_column_name(request, user_ident=request.json.get('email'))
            if email_unique:
                status = 412
                ret_val['message'] = 'Email does not exists'
            else:
                verification_code = ''.join(random.choice('0123456789ABCDEF') for i in range(6))
                user_data = await update_user_element_verification_code(request, email_id=request.json.get('email'), verification_code=str(verification_code))
                if user_data:
                    # attachments = {}
                    # await app.send_email(
                    #     targetlist=request.json.get('email'),
                    #     subject="Verification code",
                    #     content="<html><body>Your verification code is " + verification_code + "</body></html>",
                    #     attachments=attachments
                    # )
                    ret_val['success'] = True
                    status = 202
                    ret_val['message'] = 'Verification Code has been sent to your email'
        except Exception as rer_err:
            logger.error('Function api_user -> POST erred with: {}'.format(rer_err))
    return raw(ujson.dumps(ret_val).encode(), status=status, content_type='application\\json')    

@api_user_management_blueprint.route('/validate_otp', methods=['POST'])
async def api_user_validate_otp(request: Request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if email_unique:
                status = 412
                ret_val['message'] = 'User does not exists'
            else:
                user_data = await check_user_element_verification_code(request, email_id=request.json.get('email'), verification_code=request.json.get('otp'))
                if user_data:
                    ret_val['success'] = True
                    status = 202
                    ret_val['message'] = 'Otp verified successfully.'
                    ret_val['data'] = user_data
        except Exception as rer_err:
            logger.error('Function api_user -> POST erred with: {}'.format(rer_err))
    return raw(ujson.dumps(ret_val).encode(), status=status, content_type='application\\json') 

@api_user_management_blueprint.route('/reset_password', methods=['POST'])
async def api_user_reset_password(request: Request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if email_unique:
                ...
                crypted_password = await generate_password(request, new_password)
                await update_password(request, request.json.get('email'), str(crypted_password))
                ret_val['success'] = True
                status = 202
                ret_val['message'] = 'password reset successfully.'
        except Exception as rer_err:
            logger.error('Function api_user -> POST erred with: {}'.format(rer_err))
    return raw(ujson.dumps(ret_val).encode(), status=status, content_type='application\\json')    

@api_user_management_blueprint.route('/test_user', methods=['GET'])
async def api_user_test(request: Request):
    """ send otp user email
    :param request:
    :return:
    """
    status = 500
    ret_val = {'success': False, 'message': 'hiioooo', 'data': None}
    # emaildata = await compose_email("test email", "test subject", "augursmail@gmail.com" ,"096.aditya@gmail.com")
    # sd = await send_email(emaildata, 'smtp.gmail.com', 587, True, 'augursmail@gmail.com', 'Augurs@321')
    # ret_val['data'] = emaildata
    return raw(ujson.dumps(ret_val).encode(), status=status, content_type='application\\json')     

    

@api_user_management_blueprint.route('/update_profile', methods=['POST'])
@inject_user()
@scoped(['user:update'], require_all=True, require_all_actions=True)
async def api_user_element_put(request: Request, user, user_id: int = 0):
    """ Get User element / Update User element / Delete User element.
        ...
        try:
            if user:
                if user.get('user_id', None):                  
                    ...
                    webpage = request.json.get('webpage', None)
                    vatid = request.json.get('vatid', None)
                    distance_unit = request.json.get('distance_unit', "MILES")
                    fullname = request.json.get('fullname', None)
                    ...
                    if None not in [user_id]:
                        user_element = await update_user_profile(
                            request, user_id=user_id, address=address,
                            city=city, companyname=companyname, country=country, gendar=gendar,
                            postalcode=postalcode, webpage=webpage, vatid=vatid, distance_unit=distance_unit, fullname=fullname
                            )
