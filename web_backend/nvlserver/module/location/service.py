    ...
    try:
        ...
        async with request.app.pg.acquire() as connection:
            row = await connection.fetchrow(
                query_str, name, location_type_id, user_id, ujson.dumps(meta_information), show_on_map, active, end_zone)
            if row is not None:
                ret_val = dict(row)
    except Exception as gclcerr:
...
async def update_location_element(
    ...
    try:
        ...
        print(location_id, name, location_type_id, user_id, meta_information, show_on_map, active, end_zone)
        async with request.app.pg.acquire() as connection:
            row = await connection.fetchrow(
                query_str, location_id, name, location_type_id, user_id,
                ujson.dumps(meta_information), show_on_map, active, end_zone)
            if row is not None:
                ret_val = dict(row)
    except Exception as gclcerr:
