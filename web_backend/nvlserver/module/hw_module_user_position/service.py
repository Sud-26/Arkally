...
async def get_hw_module_user_position_last_point_list(
    ...
    try:
        ...
        print('khalid',hw_ids,hw_module_id_list),
        if hw_ids:
            # TODO: locking
            t = time.time()
            data = []
            ...
            if data:
                ...
                for row in filtered_data:
                    ...
                    if row.get('alarm_pause_time') is not None:
                        date_time_obj =datetime.datetime.strptime(row.get('alarm_pause_time'),'%Y-%m-%d %H:%M:%S')
                        current_time=datetime.datetime.now() 
                        time_delta = (current_time - date_time_obj)
                        total_seconds = time_delta.total_seconds()
                        alarm_pause_duration = total_seconds+row.get('alarm_pause_duration')
                    else:
                        alarm_pause_duration = row.get('alarm_pause_duration')
                    data=row.get('data')
                    if row.get('gprs_active') is False:
                       data['gps_active']=False
                       data['vehicle_running']=False 
                    transcribed_list.append(
                        Feature(geometry=row.get('position'),
                                properties={
                                    'hw_module_id': row.get('hw_module_id'),
                                    'gprs_active': row.get('gprs_active'),
                                    'data': data,
                                    'alarm_on_tme': row.get('alarm_on_tme'),
                                    'alarm_pause_time': row.get('alarm_pause_time'),
                                    'device_id':row.get('device_id'),
                                    'alarm_start': row.get('alarm_start'),
                                    'alarm_pause_status': row.get('alarm_pause_status'),
                                    'alarm_pause_duration': alarm_pause_duration,
                                    'disable_engine_start': row.get('disable_engine_start'),
                                    'stop_engine': row.get('stop_engine'),
                                    'buzzer_on': row.get('buzzer_on'),
