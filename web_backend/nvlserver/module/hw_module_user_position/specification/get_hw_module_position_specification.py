...
get_hw_module_user_last_point_element_query_view = """
...
LEFT OUTER JOIN public.vehicle_timer_time_slots AS vtts ON vtts.trraceable_object_id = tob.id
WHERE hmup.deleted IS FALSE
...
order by hmups.record_time desc ;
"""
get_hw_module_user_last_point_element_query_new = """
select  hmups.hw_module_id,
       tob.name AS vehicle_name,
       tob.alarm_pause_time AS alarm_pause_time,
       tob.alarm_start AS alarm_start,
       tob.alarm_state AS alarm_state,
       tob.alarm_pause_status,
       vtts.slots,
       tob.alarm_pause_duration,
       tob.id AS device_id,
       coalesce(to_char((tob.alarm_on_tme), 'YYYY-MM-DD HH24:MI:SS'), NULL)      AS alarm_on_tme,
       (now() - hmups.created_on)::interval < '5 seconds'::interval AS gprs_active,
       (jsonb_set(tob.meta_information,
        '{action_list}',
        hmcs.meta_information->'action_list'))::json as vehicle_data,
       ST_FlipCoordinates(hmups.position) as position,
       hmups.meta_information::json as data
from public.hw_module_user_position AS hmups
LEFT OUTER JOIN public.traceable_object AS tob ON tob.id = hmups.traceable_object_id
LEFT OUTER JOIN public.hw_module_command_state AS hmcs ON hmcs.traceable_object_id = tob.id
LEFT OUTER JOIN public.vehicle_timer_time_slots AS vtts ON vtts.trraceable_object_id = tob.id
WHERE hmups.deleted IS FALSE
  AND hmups.active IS TRUE
  AND tob.show_on_map IS TRUE
  AND (hmups.meta_information->>'gps_active')::boolean is true
  AND ($2::BIGINT is NULL OR hmups.user_id = $2::BIGINT)
  AND hmups.hw_module_id = any($1::integer[])
order by hmups.created_on desc Limit 1;
"""
...
from (select hmup.hw_module_id, hmup.position
      ...
      WHERE hmup.deleted IS FALSE
        AND hmup.active IS TRUE
        AND ($1::BIGINT is NULL OR hmup.user_id = $1::BIGINT)
        AND hmup.traceable_object_id = any ($2)
        AND (hmup.meta_information->>'gps_active')::boolean is true
