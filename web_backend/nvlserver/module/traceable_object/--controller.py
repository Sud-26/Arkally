...
from .service import (
    ...
    get_traceable_object_brand_dropdown_list, get_traceable_object_model_dropdown_list, update_traceable_object_alram_time_element, add_vehicle_wise_rent_statics,get_vehicle_wise_rent_statics_count
)
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                        
                        ...
                        if None not in [alarm_start]:
                            traceable_object = await add_vehicle_wise_rent_statics(
                                request, alarm_start=alarm_on_tme, alarm_on_tme=alarm_start, traceable_object_id=traceable_object_id,alarm_state=alarm_state,user_id=user.get('user_id'))
...
@api_traceable_object_blueprint.route('/update_alram_on/<traceable_object_id:int>', methods=['PUT'])
@inject_user()
@scoped(['traceable_object:update'], require_all=True, require_all_actions=True)
async def api_traceable_object_element_alarmput_put(request: Request, user, traceable_object_id: int = 0):
    """
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                       
                        ...
                        if None not in [alarm_on_tme]: 
                            ...
                            #if created_command:                       
                            #    traceable_object = await update_traceable_object_alram_time_element(
                            #            request, alarm_start=0, alarm_on_tme=alarm_on_tme, traceable_object_id=traceable_object_id,alarm_state=True)
                            #if traceable_object:
                            ret_val['success'] = True
                            ret_val['message'] = 'server.query_success'
                            ret_val['data'] = {}
        ...
        try:
            ...
            for device in traceable_list:
                ...
                if alarm_start:                    
                    ...
                    alrmbeforetime=date_time_obj+timedelta(hours=int(0), minutes=int(alarm_start))
                    hw_action_id = 6 #SOUND BUZZER
                    value = True
                    if (datetime.datetime.now()>=alrmbeforetime) and alarm_status is True:
                        hw_action = await get_hw_action_element(request, hw_action_id)
                        ...
                        if created_command:
                            alrmbeforetime=date_time_obj+timedelta(hours=int(5), minutes=int(alarm_start))
