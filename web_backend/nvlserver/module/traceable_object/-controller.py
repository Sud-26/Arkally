            ...
            for device in traceable_list:
                ...
                if alarm_start and device['alarm_on_tme'] is not None:
                    ...
                    if device["slots"] is not None:
                        ...
                        for (key, value) in alarrmTimeBefore.items():
                            if value==alarm_start:
                                alarrmTimeBefore=int(key)
                                print("key: {} | value: {}".format(key, value))
                    
                        print(alarrmTimeBefore,"alarrmTimeBefore")
                    ...
                    if (datetime.datetime.now()>=alrmbeforetime) and alarm_status is True and alarm_pause_status is False:
                        ...
                        if created_command:
                            alrmbeforetime=date_time_obj+timedelta(hours=int(0), minutes=int(alarm_start+3000), seconds=int(alarm_pause_duration))
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                        
                            ...
                            if traceable_object["id"]:
                                ...
                                traceable_object = await update_traceable_object_buzzer_on_off(request, traceable_object_id=traceable_object_id ,buzzer_on=value)
