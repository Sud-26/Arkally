...
from .service import (
    ...
    get_traceable_object_element, update_traceable_object_element, delete_traceable_object_element,get_vehicle_wise_rent_statics,update_traceable_object_buzzer_on_off,
    update_traceable_object_command_device,get_vehicle_wise_rent_statics_report,get_vehicle_wise_rent_statics_report_count,
    # TRACEABLE OBJECT TYPE IMPORTS
    get_traceable_object_type_list, get_traceable_object_type_dropdown_list,
    get_traceable_object_type_list_count, create_traceable_object_type_element,get_traceable_object_slot_dropdown_list,
...
async def api_traceable_object_list_get(request: Request, user):
    ...
    page = proc_arg_to_int(request.args.get('page', '1'), 1)
    name = request.args.get('name', None)
    traceable_object_id = proc_arg_to_int((request.args.get('traceable_object_id', None)),None)
    user_id = proc_arg_to_int(request.args.get('user_id', None),None)
    slot = proc_arg_to_int(request.args.get('slot', None),None)
    date_from = request.args.get('date_from', None)
    quick_time = request.args.get('quick_time', None)
    if date_from is not None:       
        date_from = datetime.datetime.strptime(date_from, '%Y-%m-%dT%H:%M')
    date_to = request.args.get('date_to', None)
    if date_to is not None:        
        date_to = datetime.datetime.strptime(date_to, '%Y-%m-%dT%H:%M')
    ...
    if quick_time is not None:        
        if quick_time == 'Today':
            ...
            date_from = dt
        if quick_time == 'Yesterday':
            dt = datetime.datetime.now()
            dt = dt.replace(hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 1)      
            date_from = yesterday
            dt = datetime.datetime.now()
            dt = dt.replace(hour=23, minute=59, second=59, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 1)           
            date_to = yesterday
        if quick_time == "Last Three day":
            dt = datetime.datetime.now()
            dt = dt.replace(hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 3)      
            date_from = dt
        if quick_time == "Last 7 Days":
            dt = datetime.datetime.now()
            dt = dt.replace(hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 7)      
            date_from = dt
        if quick_time == "This month":
            dt = datetime.datetime.now()
            dt = dt.replace(day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy 
            date_from = dt           
        if quick_time == "One Year":
            dt = datetime.datetime.now()
            dt = dt.replace(month=1,day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy                 
            date_from = dt
        if quick_time == "Five Year":
            dt = datetime.datetime.now()
            dt = dt.replace(month=1,day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 1825)      
            date_from = dt    
    offset = (page - 1) * size
                ...
                if user.get('user_id', None):
                    if user.get('account_type_name') == 'admin':
                        traceable_object_list = await get_vehicle_wise_rent_statics(
                            request, user_id=user_id, name=name, limit=size, offset=offset, traceable_object_id=traceable_object_id, slot=slot, date_from=date_from, date_to=date_to)
                        traceable_object_count = await get_vehicle_wise_rent_statics_count(
                            request, user_id=user_id, name=name, traceable_object_id=traceable_object_id, slot=slot, date_from=date_from, date_to=date_to)
                    else:
                        traceable_object_list = await get_vehicle_wise_rent_statics(
                            request, user_id=user.get('user_id', None), name=name, limit=size, offset=offset, traceable_object_id=traceable_object_id, slot=slot, date_from=date_from, date_to=date_to)
                        traceable_object_count = await get_vehicle_wise_rent_statics_count(
                            request, user_id=user.get('user_id', None), name=name, traceable_object_id=traceable_object_id, slot=slot, date_from=date_from, date_to=date_to)
...
@api_traceable_object_blueprint.route('/vehicle_wise_rent_statics_report', methods=['GET'])
@inject_user()
...
async def api_traceable_object_list_get(request: Request, user):
    ...
    traceable_object_id = proc_arg_to_int((request.args.get('traceable_object_id', None)),None)
    user_id = proc_arg_to_int(request.args.get('user_id', None),None)    
    date_from = request.args.get('quick_time', None)
    date_to = None
    if date_from is not None:        
        if date_from == 'Today':
            dt = datetime.datetime.now()
        ...
        if date_from == 'Yesterday':
            ...
            yesterday = dt - datetime.timedelta(days = 1)           
            date_to = yesterday
        if date_from == "Last Three day":
            dt = datetime.datetime.now()
            dt = dt.replace(hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 3)      
            date_from = dt
        if date_from == "Last 7 Days":
            dt = datetime.datetime.now()
            dt = dt.replace(hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 7)      
            date_from = dt
        if date_from == "This month":
            dt = datetime.datetime.now()
            dt = dt.replace(day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy 
            date_from = dt           
        if date_from == "One Year":
            dt = datetime.datetime.now()
            dt = dt.replace(month=1,day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy                 
            date_from = dt
        if date_from == "Five Year":
            dt = datetime.datetime.now()
            dt = dt.replace(month=1,day=1,hour=0, minute=0, second=0, microsecond=0) # Returns a copy
            yesterday = dt - datetime.timedelta(days = 1825)      
            date_from = dt
    ...
    if request.method == 'GET':
        try:
            if user:
                if user.get('user_id', None):
                    if user.get('account_type_name') == 'admin':
                        traceable_object_list = await get_vehicle_wise_rent_statics_report(
                            request, user_id=user_id, limit=size, offset=offset, traceable_object_id=traceable_object_id, date_from=date_from, date_to=date_to)
                        traceable_object_count = await get_vehicle_wise_rent_statics_report_count(
                            request, user_id=user_id, traceable_object_id=traceable_object_id, date_from=date_from , date_to=date_to)
                    else:
                        traceable_object_list = await get_vehicle_wise_rent_statics_report(
                            request, user_id=user.get('user_id', None),  limit=size, offset=offset, traceable_object_id=traceable_object_id,  date_from=date_from, date_to=date_to)
                        traceable_object_count = await get_vehicle_wise_rent_statics_report_count(
                            request, user_id=user.get('user_id', None),  traceable_object_id=traceable_object_id, date_from=date_from , date_to=date_to)
                    ...
                    if traceable_object_list:
                        ...
                        ret_val['date_to'] = date_to
                        ret_val['success'] = True
                        ret_val['message'] = 'server.query_error'
                        ret_val['data'] = {}
                        status = 200
                else:
                    status = 400
...
async def api_traceable_object_list_dropdown_get(request: Request, user):
    ...
    #print('user_id_khalid',request.args.get('user_id'),user)
    if request.method == 'GET':
        try:
            if user:
                if user.get('user_id', None):
                    if user.get('account_type_name') == 'admin':
                        ...
                        user_idd = user_id
                    print('account_type_name',user.get('account_type_name'),user_id)
    ...
    if request.method == 'GET':
        try:
            ...
            for device in traceable_list:
                ...
                cron_status=device['cron_status']
                if alarm_pause_duration is None:
                    alarm_pause_duration=0                
                traceable_object_id=device['id']
                
                if alarm_start and device['alarm_on_tme'] is not None:
                    #print(device) 
                   
                    alarrmTimeBefore=2 #before alamr start
                    alarmDuration=2 #alamr duration 

                    if device["slots"] is not None:
                        alarrmTimeBefore=json.loads(device["slots"])                        
                        for (key, value) in alarrmTimeBefore.items():
                            if value==alarm_start:                               
                                alrm_mtn = key.split("_")
                                if len(alrm_mtn)>1:
                                    alarrmTimeBefore=int(alrm_mtn[0])
                                    alarmDuration=int(alrm_mtn[1])
                                    if alarmDuration==0:
                                        alarmDuration=60
                    ...
                    if (datetime.datetime.now()>=alrmbeforetime) and alarm_status is True and alarm_pause_status is False:
                        ...
                        if cron_status is None:
                            ...
                            # Alarm on if all condtion full fill
                            created_command = await create_user_hw_command_element(
                                                    request, user_id=1,
                                                    hw_action_id=hw_action.get('id'), proto_field=hw_action.get('proto_field'),
                                                    field_type=hw_action.get('action').get('type'), value=str(1),
                                                    state='pending', traceable_object_id=hw_module_element.get('traceable_object_id'),
                                                    hw_module_id=hw_module_element.get('id'), ack_message=True,
                                                    active=True
                                                )
                            ...
                            if created_command:
                                traceable_object = await update_traceable_object_buzzer_on_off(request, traceable_object_id=device['id'] ,buzzer_on=value,cronstatus=True,cronValue='ON')
                                
                                           
                        #if alarm already on by cron so if timeslot completed it will off
                        alrmaftertime=date_time_obj+timedelta(hours=int(0), minutes=int(alaram_before+alarmDuration), seconds=int(alarm_pause_duration))
                       
                        if (datetime.datetime.now()>=alrmaftertime):
                            print("cron_status",cron_status,"alarmDuration",alarmDuration,alrmaftertime)
                            if 'ON' in cron_status:                                
                                value=False
                                hw_action = await get_hw_action_element(request, hw_action_id)
                                hw_module_element = await get_hw_module_element_by_traceable_object_id(request, user_id=device['user_id'], traceable_object_id=device['id'])
                                created_command = await create_user_hw_command_element(
                                                        request, user_id=1,
                                                        hw_action_id=hw_action.get('id'), proto_field=hw_action.get('proto_field'),
                                                        field_type=hw_action.get('action').get('type'), value=str(0),
                                                        state='pending', traceable_object_id=hw_module_element.get('traceable_object_id'),
                                                        hw_module_id=hw_module_element.get('id'), ack_message=True,
                                                        active=True
                                                    )
                                if created_command:
                                    traceable_object = await update_traceable_object_buzzer_on_off(request, traceable_object_id=device['id'] ,buzzer_on=value,cronstatus=True,cronValue="OFF")
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                        
                            ...
                            if traceable_object["alarm_state"]:
                                ...
                                active_duration = convertMinst(round(total_seconds))
                                total_duration=currentTime-alarm_on_tme
                                total_duration = total_duration.total_seconds()
                                total_duration = convertMinst(round(total_duration))
                                #minutes = total_seconds/60
                                extendTime=total_seconds-(traceable_object["alarm_start"]*60)
                                #extendTimeinseond=extendTime*60
                                extendTime=round(extendTime)
                                #print("extendTime",extendTime)
                                extendTimeinseond=convertMinst(extendTime)
                                #print(extendTimeinseond,'extendTimeinseond')
                                #add rent statics for completed rent
                                alarm_pause_duration=convertMinst(round(traceable_object['alarm_pause_duration']))
                                end_time=datetime.datetime.now()
                                traceable_object2 = await add_vehicle_wise_rent_statics(
                                request, alarm_start=traceable_object["alarm_start"], alarm_on_tme=alarm_on_tme, traceable_object_id=traceable_object_id,alarm_state=traceable_object["alarm_state"],user_id=user.get('user_id'),extended_time=extendTimeinseond,alarm_pause_duration=alarm_pause_duration,end_time=end_time,total_duration=total_duration,active_duration=active_duration)
                                ...
                                created_command = await create_user_hw_command_element(
                                                ...
                                                hw_action_id=hw_action.get('id'), proto_field=hw_action.get('proto_field'),
                                                field_type=hw_action.get('action').get('type'), value=str(value),
                                                state='pending', traceable_object_id=traceable_object_id,
                                                hw_module_id=hw_module_element.get('id'), ack_message=True,
                                                active=True
                                            )
                                ret_val['success'] = True
...
@api_traceable_object_blueprint.route('/rent_reset/<traceable_object_id:int>', methods=['GET'])
@inject_user()
@scoped(['traceable_object:update'], require_all=True, require_all_actions=True)
async def api_traceable_object_element_rent_reset(request: Request, user, traceable_object_id: int = 0):
    """
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                        
                            ...
                            if traceable_object["alarm_state"]:
                                ...
                                #print(extendTimeinseond,'extendTimeinseond')
                                #add rent statics for completed rent
                                #0traceable_object2 = await add_vehicle_wise_rent_statics(
                                #request, alarm_start=traceable_object["alarm_start"], alarm_on_tme=alarm_on_tme, traceable_object_id=traceable_object_id,alarm_state=traceable_object["alarm_state"],user_id=user.get('user_id'),extended_time=extendTimeinseond)
                                #alarm reset on table
                                traceable_object = await update_traceable_object_alram_time_element(
                                    request, alarm_start=traceable_object["alarm_start"], alarm_on_tme=None, traceable_object_id=traceable_object_id,alarm_state=False)
                                hw_action_id = 6 #SOUND BUZZER
...
@api_traceable_object_blueprint.route('/SOUND_BUZZER_Toggle/<traceable_object_id:int>', methods=['GET'])
@inject_user()
@scoped(['traceable_object:update'], require_all=True, require_all_actions=True)
async def api_traceable_object_element_SOUND_BUZZER_Toggle(request: Request, user, traceable_object_id: int = 0):
    """
        ...
        try:
            if user:
                ...
                if user.get('user_id'):
                    ...
                    if traceable_object_id:                        
                            traceable_object=await get_traceable_object_element(request, user_id=user.get('user_id'), traceable_object_id=traceable_object_id)
                            if traceable_object["id"]:
                                if traceable_object["buzzer_on"] is True:
                                    value = False
                                    ValueS= 0
                                else:
                                    value = True
                                ...
                                created_command = await create_user_hw_command_element(
                                                request, user_id=user.get('user_id'),
                                                hw_action_id=hw_action.get('id'), proto_field=hw_action.get('proto_field'),
                                                field_type=hw_action.get('action').get('type'), value=str(ValueS),
                                                state='pending', traceable_object_id=traceable_object_id,
                                                hw_module_id=hw_module_element.get('id'), ack_message=True,
                                                active=True
                                            )                                
                               
                                if traceable_object["cron_status"] is not None and 'ON' in traceable_object["cron_status"] and value is False:
                                    traceable_object = await update_traceable_object_buzzer_on_off(request, traceable_object_id=traceable_object_id ,buzzer_on=value,cronstatus=True,cronValue="OFF")
                                else:
                                    traceable_object = await update_traceable_object_buzzer_on_off(request, traceable_object_id=traceable_object_id ,buzzer_on=value,cronstatus=False,cronValue="")
                                ret_val['success'] = True
                                ret_val['message'] = 'server.query_success'
                                ret_val['data'] = traceable_object
                                status = 202
...
@api_traceable_object_blueprint.route('/updated_device_command/<traceable_object_id:int>', methods=['POST'])
@inject_user()
@scoped(['traceable_object:create_command'], require_all=True, require_all_actions=True)
async def api_traceable_object_updated_device_command(request: Request, user, traceable_object_id: int = 0):
    """
    :param traceable_object_id: 
    :param request:
    :param user:
        ...
        try:
            if user:
                if user.get('user_id'):
                    ...
                    # update device code 
                    if traceable_object["id"]:
                        #DISABLE ENGINE START action
                        if hw_action_id==5:
                            traceable_object=await update_traceable_object_command_device(request,traceable_object_id=traceable_object_id,disable_engine_start=value,stop_engine=traceable_object["stop_engine"])
                        #STOP ENGINE action
                        if hw_action_id==4:
                            traceable_object=await update_traceable_object_command_device(request,traceable_object_id=traceable_object_id,disable_engine_start=traceable_object["disable_engine_start"],stop_engine=value)
...
# TRACEABLE OBJECT SLOT API
@api_traceable_object_blueprint.route('/slot/dropdown', methods=['GET'])
@inject_user()
@scoped(['traceable_object:query_dropdown'], require_all=True, require_all_actions=True)
async def api_traceable_object_slot_list_get(request: Request, user):
    """
    ...
    if request.method == 'GET':
        try:
            if user:
                if user.get('user_id', None):
                    traceable_object_slot_list = await get_traceable_object_slot_dropdown_list(
                        request, name=name)
                    slotlist=[]
                    
                    if traceable_object_slot_list:
                        for row in traceable_object_slot_list:
                            person_dict = json.loads(row['slots'])               
                            for key in person_dict:                                
                                if key is not None:
                                    xd = key.split("_")                                    
                                    if person_dict[key]!="" and int(float(person_dict[key])) not in slotlist:                                                                                
                                            slotlist.append((int(float(person_dict[key]))))
                        slotlist.sort()
                        ret_val['success'] = True
                        ret_val['message'] = 'server.query_success'
                        ret_val['data'] = slotlist
                        status = 200
                    else:
                        ret_val['success'] = True
                        ret_val['message'] = 'server.query_success'
