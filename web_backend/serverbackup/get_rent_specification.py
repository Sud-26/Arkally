...
SELECT rnt.id                                                                     AS id,
       ...
       rnt.alaram_time  AS alaram_time,
       rnt.active                                                                 AS active,
...
get_alarm_timein_query = """
SELECT id, user_id, hw_action_id, proto_field, field_type, value, active, deleted, created_on, updated_on, ack_message, hw_module_id, state, date_from,TO_CHAR(date_to,'YYYY-MM-DD HH24:MI:SS')   AS date_to , traceable_object_id, alaram_time, alarm_status
	FROM public.user_hw_command As rnt where date_from<=now() and date_to>=now() and alarm_status=false
""" 