...
# INSERT QUERY IF NULL RECIVED IN TO HW MODULE USER POSITION VIEW
create_hw_module_user_position_element_nullLatLong_query = """
INSERT INTO hw_module_user_position_view
(user_id, traceable_object_id, hw_module_id, position, raw_nmea,
 meta_information, show_on_map, active, deleted, created_on, updated_on, record_time)
SELECT user_id,traceable_object_id,hw_module_id,position, %s, %s,show_on_map, TRUE, FALSE, now(), now(), now() FROM  hw_module_user_position_view AS hmv 
WHERE hw_module_id=(SELECT hm.id FROM  public.hw_module AS hm WHERE hm.module_id =  %s)
...
SELECT %s,%s,%s,%s,%s,%s,%s,%s,%s,end_zone,id FROM location WHERE
...
UPDATE public.traceable_object
	SET  alarm_on_tme=%s, alarm_state=False, buzzer_on=False
	WHERE id = %s
"""