...
class Echo(Protocol):
    ...
    def trigger_action_on_intersect_detect(self, data):
        ...
        if len(data) > 0:
            print('========== INTERSECTED ACTION CHECK IF IT END LOCATION ==========\n')       
            if self.HW_MOD_ID is not None:
                    command_data_end_location = conn.runQuery(query_find_alarm_on_for_device, (self.HW_MOD_ID,))                
                    command_data_end_location.addCallback(self.trigger_action_on_intersect_detect_check_end_location)
            print(data)             
            for action in data:
                ...
                if action[3]==6:
                          time.sleep(1)
                          intersect_detect = conn.runQuery(
                    create_hw_command_element_on_intersect_query, (action[3], action[2], 0, self.HW_MOD_ID,))


    def convertMinst(self,seconds):
        secondsG=seconds
        if(seconds<0):
            seconds=abs(seconds)     
    ...
    def trigger_action_on_intersect_detect_check_end_location(self, data):
        """ check if alarm on and it is in end location
        """
        print('========== INTERSECTED INSERT RENT STATICS CHECK IF END LOCATION AND ALARM ON ==========\n')
        
        #print(data[0][0])
        if len(data) > 0:            
            alarm_on_tme=datetime.datetime.strptime(data[0][6], '%Y-%m-%dT%H:%M:%S')
            pause_time=data[0][5]
            alarm_time_sltos=data[0][4]
            traceble_object_id=data[0][3]
            traceble_user_id=data[0][2]
            currentTime=datetime.datetime.now()
            timediff=currentTime-alarm_on_tme
            
            #total_seconds = timediff.total_seconds()
            #minus second pause time
            total_seconds = timediff.total_seconds()
            active_duration = self.convertMinst(round(total_seconds))
            total_duration=currentTime-alarm_on_tme
            total_duration = total_duration.total_seconds()            
            total_duration = self.convertMinst(round(total_duration))
            total_timer_spend_time=total_seconds-pause_time
            total_timer_spend_time_minutes = total_timer_spend_time/60
            extended_time_after_slots_completed=total_timer_spend_time_minutes-alarm_time_sltos
            print("timer completed2")
            #extended_time_after_slots_completed=round(extended_time_after_slots_completed, 2)
            extended_time_after_slots_completed=extended_time_after_slots_completed*60
            #if extended_time_after_slots_completed>60:
            extended_time_after_slots_completed=self.convertMinst(round(extended_time_after_slots_completed,0))
            #else:
            #    extended_time_after_slots_completed=extended_time_after_slots_completed
            #check percent timer completed
            timer_spend_in_percent=(total_timer_spend_time_minutes*60)/(alarm_time_sltos*60)
            timer_spend_in_percent=timer_spend_in_percent*100
            alarm_pause_duration=self.convertMinst(round(pause_time))
            end_time=datetime.datetime.now()            
            if timer_spend_in_percent>90:
                command_data_end_location = conn.runQuery(query_find_end_location_for_device, (alarm_time_sltos,alarm_on_tme,
                traceble_object_id,traceble_user_id,extended_time_after_slots_completed,alarm_pause_duration,end_time,total_duration,active_duration,data[0][0]))                
                command_data_end_location.addCallback(self.trigger_action_on_intersect_detect_check_end_location_timer_start)

    def trigger_action_on_intersect_detect_check_end_location_timer_start(self, data):
        """ check if alarm on and it is in end location
        """        
        if len(data) > 0:
            
            print('========== INTERSECTED INSERT RENT STATICS CHECK IF END LOCATION ==========\n')
            alarm_time_sltos=data[0][0]
            alarm_on_tme=data[0][1]
            traceble_object_id=data[0][2]
            traceble_user_id=data[0][3]
            extended_time_after_slots_completed=data[0][4]
            alarm_pause_duration=data[0][5]
            end_time=data[0][6]
            total_duration=data[0][7]
            active_duration=data[0][8]
            
            #Sound off alarm buzzer
            intersect_detect = conn.runQuery(
                    create_hw_command_element_on_intersect_query, (6, 'sound_buzzer', False, self.HW_MOD_ID,))
            
            command_data_end_location = conn.runQuery(create_vehicle_wise_rent_statics, (traceble_object_id,alarm_on_tme,
                alarm_time_sltos,traceble_user_id,extended_time_after_slots_completed,alarm_pause_duration,end_time,total_duration,active_duration,))  
            #print(command_data_end_location)
            updatealrm = conn.runQuery(update_trcable_object_alaram_rent_status, (None,traceble_object_id,))
            #print(data)
    ...
    def dataReceived(self, data):
        ...
        try:            
            ...
            if message_id == 0xB:
                ...
                print('khalid',gpsstatus.longitude)
                if gpsstatus.longitude!=gpsstatus.longitude:                    
                    print('nolatlong')
                    longitude = 18.167272567749023
                    latitude = 48.643775939941406
                else:
                ...
                if gpsstatus.longitude:
                    print('nan call',gpsstatus.longitude,gpsstatus.latitude)
                    hw_mod_user_pos_data = conn.runQuery(
                    create_hw_module_user_position_element_query, value_tuple)
                    #hw_mod_user_pos_data = conn.runQuery(
                    #    create_hw_module_user_position_element_nullLatLong_query, value_tuple_Lat_LONG_NULL)
                else:
                    hw_mod_user_pos_data = conn.runQuery(
                    create_hw_module_user_position_element_query, value_tuple)
